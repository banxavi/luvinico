...
export function getSearchFilterOptions(products, { includeTypes = false, catalog } = {}) {
  const origins = [...new Set((products ?? []).map((p) => p.origin).filter(Boolean))].sort((a, b) =>
    a.localeCompare(b, 'vi'),
  );

  const categoriesRecord = catalog?.categories ?? {};
  const productTypesRecord = catalog?.productTypes ?? {};

  const categoryKeys = [
    ...new Set((products ?? []).map((p) => getProductCategory(p)).filter((key) => categoriesRecord[key])),
  ].sort((a, b) => categoriesRecord[a].title.localeCompare(categoriesRecord[b].title, 'vi'));

  const categories = categoryKeys.map((key) => ({
    value: key,
    label: categoriesRecord[key].title,
  }));

  const types = includeTypes
    ? [...new Set((products ?? []).map((p) => getProductType(p)).filter(Boolean))].map((slug) => ({
        value: slug,
        label: productTypesRecord[slug]?.label ?? slug,
      }))
    : [];
